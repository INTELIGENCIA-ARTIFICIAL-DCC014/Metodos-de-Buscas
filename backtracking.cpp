...
// Função recursiva que implementa o algoritmo de backtracking para encontrar a solução
void Grafo::backtracking(vector<int>& rota, int pos, int valor, int& solAchada, vector<int>& rotaAchada, int& nosVisitados) {
    nosVisitados++;

    // Verifica se encontrou um ciclo (impasse), ou seja, se o vértice já foi visitado
    ...
    // Tenta adicionar cada vértice na posição 'pos' da rota e chama recursivamente para o próximo vértice
